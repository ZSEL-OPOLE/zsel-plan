...
class Instance:
    ...
    teacher_unavail: dict[str, set[tuple[str, int]]] = field(
        default_factory=dict
    )  # nauczyciel -> {(dzien,period) niedostępne}
...
class Result:
    ...
    timetable: dict[tuple[str, int, int], tuple[str, str, str]] = field(
        default_factory=dict
    )
    gaps: int = 0
...
def solve(inst: Instance, max_seconds: int = 30) -> Result:
    ...
    for kl in {lesson[1] for lesson in lessons}:
        for s in range(len(slots)):
            m.Add(
                sum(v for (i, ss, r), v in x.items() if ss == s and lessons[i][1] == kl)
                <= 1
            )
    # nauczyciel: max 1 na slot
    for naucz in {lesson[3] for lesson in lessons}:
        for s in range(len(slots)):
            m.Add(
                sum(
                    v
                    for (i, ss, r), v in x.items()
                    if ss == s and lessons[i][3] == naucz
                )
                <= 1
            )
    # sala: max 1 na slot
    for r in rooms:
        for s in range(len(slots)):
            m.Add(sum(v for (i, ss, rr), v in x.items() if ss == s and rr == r) <= 1)
    ...
    for kl in {lesson[1] for lesson in lessons}:
        for d in D:
            ...
            m.Add(
                span
                >= last - first + 1 - (len(P)) * (1 - m.NewBoolVar(f"any_{kl}_{d}"))
            )
